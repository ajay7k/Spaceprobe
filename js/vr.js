...
function render() {
	...
	if (intersects.length > 0) {
		...
		// 🌈 Neon pulsing hover effect
		let pulse = Math.abs(Math.sin(timer * 4)); // oscillates 0–1
		let glowColor = new THREE.Color("#00fff7"); // neon cyan
		let baseColor = new THREE.Color("#003344"); // deep teal
		let blended = baseColor.lerp(glowColor, pulse * 0.9);
		intersects[0].face.color.copy(blended);
